...
const UpcomingEvent = () => {
    ...
    const [upcomingevent, setUpcomingvent] = useState([])
    // event table
    const [row, setRow] = useState(upcomingevent)
    ...
    useEffect(() => {
        upcomingevent.sort((a, b) => new Date(b.date) - new Date(a.date))
        setRow(upcomingevent);
    }, [upcomingevent]);

    const requestSearch = (searchedVal) => {
        const filteredRows = upcomingevent.filter((event) => {
